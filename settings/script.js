...
function sv() {
  ...
  if (document.getElementById("regex").value === "") {
    document.getElementById("regex-error-text").innerText =
      "＊この項目は入力必須です";
  ...
  try {
    ...
    document.getElementById("excregex-error-text").innerText =
      "＊正規表現の書式が誤っています";
...
function rm() {
  ...
  document.getElementById("logsize").innerText =
    String(localStorage.getItem("log").length) + " バイト";
}
...
function reset() {
  ...
  localStorage.setItem(
    "exclusionURL",
    "(.*\\.)(jpg|jpeg|png|gif|svg|ico|css|js|woff|woff2|mp4|mp3|ts|m3u8|pdf|doc|docx|ppt|pptx|xls|xlsx|md)$",
  );
  ...
  document.getElementById("excregex").value =
    localStorage.getItem("exclusionURL");
...
document.getElementById("logsize").innerText =
  String(localStorage.getItem("log").length) + " バイト";
document.getElementById("logview").addEventListener("click", () => {
